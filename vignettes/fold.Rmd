...
Using `table2()`, one can calculate mean differences in `mpg`, `hp`, or `wt`:
...
---------|--------------|-----------------|-------------------------------------
`"fold"` | Arithmetric means | Generalized linear model with log link: `glm(y ~ x, family = gaussian(link = "log"))` | Tolerates `0` in the `outcome` variable. Best fit if outcome is normally distributed without transformation.
`"fold"` | Geometric means   | Linear model with log-transformed outcome: `lm(log(y) ~ x)` | Does not tolerate `0` in the `outcome` variable, as `log(0)` is undefined (R returns `-Inf`). Best fit if outcome is normally distributed after log transformation.

In both models, beta coefficients can be interpreted as ratios after exponentiation.
